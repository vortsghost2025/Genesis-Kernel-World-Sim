...
| **Rollback anchor envelope format documented** | Rollback §11: anchor must be "explicit, caller-supplied, provenanced, sanitized." | Phase 10IH now documents the exact five-field envelope format and validation rules enforced by 10IC. Phase 10IO specifies the state_commitment source-envelope schema and commitment derivation (docs-level design). Rollback execution, authorization, replay prevention, and runtime source-envelope validator remain unresolved. |
...
5. **Rollback anchor envelope format documented** — five-field envelope, exact validation rules, claim classification semantics, habitat binding documented by Phase 10IH; state_commitment source-envelope schema and declared-digest shape documented by Phase 10IO (docs-level designs complete; declared digest does not verify actual artifact bytes; last-known-good state material envelope unresolved per 10IH §7; runtime validator, rollback execution, authorization, replay prevention, and operator approval remain unresolved).
...
| Rollback state_commitment source-envelope documented | ✅ Yes — by Phase 10IO (source-envelope schema, commitment derivation; declared digest is caller-asserted shape only — actual artifact-content binding and last-known-good state envelope remain unresolved per 10IH §7) |
