...
class WorldAgent:
    ...
    def decide(self, world_snapshot: dict[str, Any]) -> dict[str, Any]:
        ...
        try:
            ...
            thought, action, goal = self._parse_response(response, world_snapshot)
    ...
    def _build_prompt(self, world: dict[str, Any]) -> str:
        ...
        prompt += (
            ...
            f'{{"thought": "your internal thought", "action": "what you do", "goal": "your current active goal or focus"}}\n'
        )

        prompt += f"\n\nWhat do you think, what is your goal, and what do you do right now? Return ONLY valid JSON."
        return prompt

    def _parse_response(self, response: str, world: dict[str, Any]) -> tuple[str, str, str]:
        """Parse provider response into thought, action, and goal."""
        if response.startswith("{"):
            try:
                ...
                if thought and action:
                    return thought, action, goal
            except json.JSONDecodeError:
                pass

        thought, action = self._mock_think_action(world)
    ...
    def remember(self, result: dict[str, Any]) -> None:
        """
        General memory entry point. 
        Processes a result from the consequence engine.
        """
        ...
        importance = 0.5
        if "critical" in content.lower() or "first" in content.lower() or "discovered" in content.lower():
            importance = 0.9
        
        self.remember_event(content, importance)
    ...
    def whisper(self, target_agent: "WorldAgent", content: str, importance: float = 0.7) -> None:
        """Send a whisper to another agent within the same hemisphere."""
        if self.region != target_agent.region:
            logger.warning(
                "%s tried to whisper across hemispheres to %s — blocked",
                self.name, target_agent.name,
            )
            return
        target_agent.persistent_memory.add_whisper(
        ...
        self.remember_relationship(
            f"Whispered to {target_agent.name}: {content[:80]}",
            importance=0.3,
        )
        logger.info("WHISPER: %s -> %s: %s", self.name, target_agent.name, content[:60])

    def receive_whispers(self) -> list[dict[str, Any]]:
        """Receive and process unread whispers, storing them as memories."""
        unread = self.persistent_memory.get_unread_whispers()
        if not unread:
            return []
        received = []
        for w in unread:
            self.remember_event(
                f"Heard {w['from']} say: {w['content']}",
                importance=w.get("importance", 0.7),
            )
            received.append(w)
        self.persistent_memory.mark_whispers_read()
        return received
    ...
    def load_state(self, path: Path) -> None:
        ...
        self.reflection_counter = data.get("reflection_counter", 0)
