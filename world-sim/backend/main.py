...
def setup_agents(cfg: WorldSimConfig) -> dict[str, WorldAgent]:
    ...
    # Define the 4 specific NPCs for the dual-hemisphere simulation
    npc_definitions = {
        "East Adam": {"factory": create_adam, "region": "east", "role": "gardener of the east"},
        "East Eve": {"factory": create_eve, "region": "east", "role": "explorer of the east"},
        "West Adam": {"factory": create_adam, "region": "west", "role": "builder of the west"},
        "West Eve": {"factory": create_eve, "region": "west", "role": "navigator of the west"},
    }

    for name, defs in npc_definitions.items():
        agent = defs["factory"]()
        agent.name = name
        agent.role = defs["role"]
        agent.region = defs["region"]
        agents[name] = agent

    # Configure providers for each agent
    for name, agent in agents.items():
        # Create a normalized env key (e.g., "East Adam" -> "AGENT_EAST_ADAM_NIM_KEY")
        env_key = f"AGENT_{name.replace(' ', '_').upper()}_NIM_KEY"
        
        # Check if we are in live mode and if a key exists
        if cfg.provider_mode == "nim-live" and os.environ.get(env_key):
            agent.set_provider(NvidiaNimProvider(
                name=f"{name.lower().replace(' ', '_')}_nim",
                api_key_env=env_key,
                model=cfg.agents.get(name).model if name in cfg.agents else "meta/llama-3.1-8b-instruct",
                mode="nim-live",
            ))
        elif cfg.provider_mode == "nim-dry-run" and os.environ.get(env_key):
            agent.set_provider(NvidiaNimProvider(
                name=f"{name.lower().replace(' ', '_')}_nim",
                api_key_env=env_key,
                model=cfg.agents.get(name).model if name in cfg.agents else "meta/llama-3.1-8b-instruct",
            ...
            # Fallback to Mock or Local Ollama (if implemented in provider)
            agent.set_provider(MockProvider(name=f"{name.lower().replace(' ', '_')}_mock"))

        # Apply traits from config if any
        if name in cfg.agents and cfg.agents[name].traits:
            agent.traits.update(cfg.agents[name].traits)
