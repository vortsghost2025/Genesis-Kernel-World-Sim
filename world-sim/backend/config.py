...
@dataclass
class VideoConfig:
    """Configuration for the HLS video pipeline."""

    enabled: bool = False
    style: str = "cinematic_biblical"
    segment_duration_s: int = 18
    image_regen_interval: int = 5
    hls_segment_dir: str = "data/stream/segments"
    hls_playlist_path: str = "data/stream/playlist.m3u8"
    narrator_voice: str = "English-US.RadTTS.Male-1"
    hls_web_path: str = "/stream"

    @staticmethod
    def from_env() -> "VideoConfig":
        return VideoConfig(
            enabled=os.environ.get("VIDEO_PIPELINE_ENABLED", "false").lower() == "true",
            style=os.environ.get("VIDEO_STYLE", "cinematic_biblical"),
            segment_duration_s=int(os.environ.get("VIDEO_SEGMENT_DURATION", "18")),
            image_regen_interval=int(os.environ.get("IMAGE_REGEN_INTERVAL", "5")),
            hls_segment_dir=os.environ.get("HLS_SEGMENT_DIR", "data/stream/segments"),
            hls_playlist_path=os.environ.get("HLS_PLAYLIST_PATH", "data/stream/playlist.m3u8"),
            narrator_voice=os.environ.get("TTS_NARRATOR_VOICE", "English-US.RadTTS.Male-1"),
            hls_web_path=os.environ.get("HLS_WEB_PATH", "/stream"),
        )
