...
Create a deterministic sanitized shared-public-first-event-id-equality
contract artifact from a Phase 10AS two-agent public merge.  10BH
formalizes whether two agents' public first event IDs are identical,
...
not read them from 10AS bundles.  Caller-supplied first event IDs are
...
:func:`contract_first_event_id_equality_file`, which only reads JSON
from a caller-supplied path.

10BH consumes 10AS only.  It never imports or calls 10AS, 10AR, 10AQ,
...
``world-sim/data``.  The only upstream symbol 10BH imports is
...
10BC, 10BD, 10BE, 10BF, and 10BG already import).
"""
...
_CONTRACT_SCHEMA_VERSION = "10BH.1"
_CONTRACT_TYPE = "shared_public_first_event_id_equality_contract"
_SOURCE_PHASE = "10AS"
_CLAIM_SCOPE = "shared_public_first_event_id_equality_only"
...
def _empty_contract(errors: list[str]) -> dict[str, Any]:
    return {
        ...
        "same_first_event_id": False,
        "shared_first_event_id": None,
...
def _sanitize_event_id(raw: Any) -> str | None:
    """Sanitize a caller-supplied first event ID.
...
def create_shared_first_event_id_equality_contract(
    merge: dict,
    *,
    agent_a_first_event_id: Any = None,
    agent_b_first_event_id: Any = None,
) -> dict:
    """Create a deterministic sanitized shared-public-first-event-id-equality contract.
    ...
    first event IDs.  Emits a thin first-event-id-equality contract
    recording whether the two agents' supplied first event IDs are
    mechanically equal.  Caller-supplied first event IDs are validated
    as non-empty strings; missing, non-string, or empty-after-sanitize
    values are treated as ``None``.  Missing or None first event ID
    ...
    # --- Caller-supplied first event IDs -----------------------------------
    ...
    a_event_id = _sanitize_event_id(agent_a_first_event_id)
    b_event_id = _sanitize_event_id(agent_b_first_event_id)

    # --- ID comparison ----------------------------------------------------
    # Pure string equality.  No semantic interpretation of ID meaning.
    # No ordering, no sequence inference, no temporal inference.

    same_first_event_id = bool(
        a_event_id is not None
        and b_event_id is not None
        and a_event_id == b_event_id
    )
    shared_first_event_id = a_event_id if same_first_event_id else None
    ...
    contract_material: dict[str, Any] = {
        ...
        "agent_b_first_event_id": b_event_id if b_event_id is not None else "",
        "same_first_event_id": same_first_event_id,
        "shared_first_event_id": (
            shared_first_event_id
            if shared_first_event_id is not None
            else ""
        ),
    }
    contract_id = "10BH-" + _hash_canonical(contract_material)[:32]
    ...
    return {
        ...
        "agent_a_first_event_id": a_event_id,
        "agent_b_first_event_id": b_event_id,
        "same_first_event_id": same_first_event_id,
        "shared_first_event_id": shared_first_event_id,
        "claim_scope": _CLAIM_SCOPE,
        "errors": [],
    }


def export_shared_first_event_id_equality_contract(contract: dict) -> str:
    """Export a first-event-id-equality contract as stable sanitized JSON."""

    sanitized = sanitize_public_mapping(contract)
    return json.dumps(sanitized, sort_keys=True, ensure_ascii=False)


def contract_first_event_id_equality_file(
    merge_json_path: Path | str,
    *,
    agent_a_first_event_id: Any = None,
    agent_b_first_event_id: Any = None,
) -> dict:
    """Read an exported Phase 10AS merge JSON file and create a first-event-id-equality contract.
    ...
    return create_shared_first_event_id_equality_contract(
        merge,
        agent_a_first_event_id=agent_a_first_event_id,
        agent_b_first_event_id=agent_b_first_event_id,
    )
