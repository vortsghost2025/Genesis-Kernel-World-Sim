...
Create a deterministic sanitized shared-public-route-intent-id-equality
contract artifact from a Phase 10AS two-agent public merge.  10BK
formalizes whether two agents' public route intent IDs are identical,
without ever inferring same route, same path, same destination, same
timing, same plan, co-presence, meeting, interaction, proximity,
awareness, or relationship.

Route intent IDs are read from the 10AS agent bundles. Missing,
...
:func:`contract_route_intent_id_equality_file`, which only reads JSON
from a caller-supplied path.

10BK consumes 10AS only.  It never imports or calls 10AS, 10AR, 10AQ,
...
``world-sim/data``.  The only upstream symbol 10BK imports is
...
_CONTRACT_SCHEMA_VERSION = "10BK.1"
_CONTRACT_TYPE = "shared_public_route_intent_id_equality_contract"
_SOURCE_PHASE = "10AS"
_CLAIM_SCOPE = "shared_public_route_intent_id_equality_only"
...
def _empty_contract(errors: list[str]) -> dict[str, Any]:
    return {
        ...
        "agent_a_id": None,
        "agent_b_id": None,
        "agent_a_route_intent_id": None,
        "agent_b_route_intent_id": None,
        "same_route_intent_id": False,
        "shared_route_intent_id": None,
        "claim_scope": _CLAIM_SCOPE,
        "errors": errors,
    }


def _sanitize_route_intent_id(raw: Any) -> str | None:
    """Sanitize a public route intent ID.
...
def create_shared_route_intent_id_equality_contract(merge: dict) -> dict:
    """Create a deterministic sanitized route-intent-id-equality contract.

    Consumes an already-built Phase 10AS two-agent public merge artifact
    and emits a thin route-intent-id-equality contract recording whether
    the two agents' public bundle route intent IDs are mechanically
    equal.  Missing, non-string, or empty-after-sanitize values are
    treated as ``None``.  Never infers same route, same path, same
    destination, same timing, same plan, co-presence, meeting,
    interaction, proximity, awareness, or relationship.  Never raises;
    ...
    # --- Bundle route intent IDs ------------------------------------------
    # These are read only from the sanitized 10AS agent bundles.  No
    # caller-supplied route-intent overrides are accepted.

    a_route_intent = _sanitize_route_intent_id(
        sanitized_agent_a.get("route_intent_id")
    )
    b_route_intent = _sanitize_route_intent_id(
        sanitized_agent_b.get("route_intent_id")
    )

    # --- ID comparison ----------------------------------------------------
    # Pure string equality.  No semantic interpretation of ID meaning.
    # No ordering, no route, no path, no destination, no timing inference.

    same_route_intent_id = bool(
        a_route_intent is not None
        and b_route_intent is not None
        and a_route_intent == b_route_intent
    )
    shared_route_intent_id = a_route_intent if same_route_intent_id else None
    ...
    contract_material: dict[str, Any] = {
        ...
        "agent_a_route_intent_id": (
            a_route_intent if a_route_intent is not None else ""
        ),
        "agent_b_route_intent_id": (
            b_route_intent if b_route_intent is not None else ""
        ),
        "same_route_intent_id": same_route_intent_id,
        "shared_route_intent_id": (
            shared_route_intent_id
            if shared_route_intent_id is not None
            else ""
        ),
    }
    contract_id = "10BK-" + _hash_canonical(contract_material)[:32]
    ...
    return {
        ...
        "agent_a_route_intent_id": a_route_intent,
        "agent_b_route_intent_id": b_route_intent,
        "same_route_intent_id": same_route_intent_id,
        "shared_route_intent_id": shared_route_intent_id,
        "claim_scope": _CLAIM_SCOPE,
        "errors": [],
    }


def export_shared_route_intent_id_equality_contract(contract: dict) -> str:
    """Export a route-intent-id-equality contract as stable sanitized JSON."""

    sanitized = sanitize_public_mapping(contract)
    return json.dumps(sanitized, sort_keys=True, ensure_ascii=False)


def contract_route_intent_id_equality_file(
    merge_json_path: Path | str,
) -> dict:
    """Read an exported Phase 10AS merge JSON file and create a route-intent-id-equality contract.
    ...
    return create_shared_route_intent_id_equality_contract(merge)
