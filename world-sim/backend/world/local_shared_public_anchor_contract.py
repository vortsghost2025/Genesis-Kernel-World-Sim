"""Phase 10AU - shared public anchor / reference sharing contract.

Create a deterministic sanitized shared-public-anchor contract artifact
from a Phase 10AS two-agent public merge.  10AU formalizes which public
landmarks, event refs, or territory refs two agents have each cited in
their already-public surfaces, without ever inferring private knowledge,
co-presence, awareness, trust, cooperation, conflict, communication, or
any kind of relationship.

Anchor identifiers come from the agent bundles' ``public_anchor_ids``
...
:func:`contract_shared_anchor_file`, which only reads JSON from a
caller-supplied path.

10AU consumes 10AS only.  It never imports or calls 10AS, 10AR, 10AQ,
...
``world-sim/data``.  The only upstream symbol 10AU imports is
``sanitize_public_mapping`` from the public egress sanitizer (the same
helper 10AR, 10AS, and 10AT already import).
"""
...
_CONTRACT_SCHEMA_VERSION = "10AU.1"
_CONTRACT_TYPE = "shared_public_anchor_contract"
_SOURCE_PHASE = "10AS"
_CLAIM_SCOPE = "shared_public_anchors_only"
...
def _resolve_anchor_ids(
    ...
    return _sorted_unique_strings(bundle.get("public_anchor_ids"))
...
def create_shared_public_anchor_contract(
    merge: dict,
    agent_a_anchor_ids: list[str] | None = None,
    agent_b_anchor_ids: list[str] | None = None,
) -> dict:
    """Create a deterministic sanitized shared-public-anchor contract.

    Consumes an already-built Phase 10AS two-agent public merge artifact
    and emits a thin shared-public-anchor contract recording which public
    anchors / references the two agents demonstrably share.  Anchor
    identifiers come from the agent bundles' ``public_anchor_ids`` fields
    unless overridden by the optional ``agent_a_anchor_ids`` /
    ``agent_b_anchor_ids`` arguments.  Never infers private knowledge,
    co-presence, awareness, trust, cooperation, conflict, communication,
    or any kind of relationship.  Never raises; always returns a dict.
    """
    ...
    # Resolve anchor ids: caller-supplied overrides take precedence,
    # otherwise fall back to the bundle's public_anchor_ids field.
    a_anchor_ids = _resolve_anchor_ids(sanitized_agent_a, agent_a_anchor_ids)
    b_anchor_ids = _resolve_anchor_ids(sanitized_agent_b, agent_b_anchor_ids)

    shared_anchor_ids = sorted(set(a_anchor_ids) & set(b_anchor_ids))
    a_only_anchor_ids = sorted(set(a_anchor_ids) - set(b_anchor_ids))
    b_only_anchor_ids = sorted(set(b_anchor_ids) - set(a_anchor_ids))
    ...
    contract_material: dict[str, Any] = {
        ...
        "shared_public_anchor_ids": shared_anchor_ids,
        "agent_a_only_anchor_ids": a_only_anchor_ids,
        "agent_b_only_anchor_ids": b_only_anchor_ids,
        "claim_scope": _CLAIM_SCOPE,
    }
    contract_id = "10AU-" + _hash_canonical(contract_material)[:32]
    ...
    return {
        ...
        "shared_public_anchor_ids": shared_anchor_ids,
        "shared_public_anchor_count": len(shared_anchor_ids),
        "agent_a_only_anchor_ids": a_only_anchor_ids,
        "agent_b_only_anchor_ids": b_only_anchor_ids,
        "claim_scope": _CLAIM_SCOPE,
        "errors": [],
...
def export_shared_public_anchor_contract(contract: dict) -> str:
    """Export a shared-public-anchor contract as stable sanitized JSON."""

    sanitized = sanitize_public_mapping(contract)
    return json.dumps(sanitized, sort_keys=True, ensure_ascii=False)


def contract_shared_anchor_file(merge_json_path: Path | str) -> dict:
    """Read an exported Phase 10AS merge JSON file and create a shared-public-anchor contract.
    ...
    return create_shared_public_anchor_contract(merge)
