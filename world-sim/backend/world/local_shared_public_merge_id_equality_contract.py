...
Create a deterministic sanitized shared-public-merge-id-equality
contract artifact from a Phase 10AS two-agent public merge.  10BI
formalizes whether two agents' public merge IDs are identical,
without ever inferring same event, same time, same sequence, same
interaction, same relationship, or same merge event.

Merge IDs are supplied as optional caller arguments; 10BI does
not read them from 10AS bundles.  Caller-supplied merge IDs are
...
:func:`contract_merge_id_equality_file`, which only reads JSON
from a caller-supplied path.

10BI consumes 10AS only.  It never imports or calls 10AS, 10AR, 10AQ,
...
``world-sim/data``.  The only upstream symbol 10BI imports is
...
10BC, 10BD, 10BE, 10BF, 10BG, and 10BH already import).
"""
...
_CONTRACT_SCHEMA_VERSION = "10BI.1"
_CONTRACT_TYPE = "shared_public_merge_id_equality_contract"
_SOURCE_PHASE = "10AS"
_CLAIM_SCOPE = "shared_public_merge_id_equality_only"
...
def _empty_contract(errors: list[str]) -> dict[str, Any]:
    return {
        ...
        "same_merge_id": False,
        "shared_merge_id": None,
        "claim_scope": _CLAIM_SCOPE,
        "errors": errors,
    }


def _sanitize_merge_id(raw: Any) -> str | None:
    """Sanitize a caller-supplied merge ID.
...
def create_shared_merge_id_equality_contract(
    merge: dict,
    *,
    agent_a_merge_id: Any = None,
    agent_b_merge_id: Any = None,
) -> dict:
    """Create a deterministic sanitized shared-public-merge-id-equality contract.
    ...
    merge IDs.  Emits a thin merge-id-equality contract recording whether
    the two agents' supplied merge IDs are mechanically equal.
    Caller-supplied merge IDs are validated as non-empty strings;
    failing or empty-after-sanitize values are treated as ``None``.
    Missing or None merge ID fields produce a contract with ok=True,
    id fields set to None, and all equality booleans set to False.
    Never infers same event, same time, same sequence, same interaction,
    same relationship, or same merge event.  Never raises; always
    returns a dict.
    """
    ...
    a_merge_id = _sanitize_merge_id(agent_a_merge_id)
    b_merge_id = _sanitize_merge_id(agent_b_merge_id)
    ...
    same_merge_id = bool(
        a_merge_id is not None
        and b_merge_id is not None
        and a_merge_id == b_merge_id
    )
    shared_merge_id = a_merge_id if same_merge_id else None
    ...
    contract_material: dict[str, Any] = {
        ...
        "agent_b_merge_id": b_merge_id if b_merge_id is not None else "",
        "same_merge_id": same_merge_id,
        "shared_merge_id": (
            shared_merge_id
            if shared_merge_id is not None
            else ""
        ),
    }
    contract_id = "10BI-" + _hash_canonical(contract_material)[:32]
    ...
    return {
        ...
        "agent_a_merge_id": a_merge_id,
        "agent_b_merge_id": b_merge_id,
        "same_merge_id": same_merge_id,
        "shared_merge_id": shared_merge_id,
        "claim_scope": _CLAIM_SCOPE,
        "errors": [],
    }


def export_shared_merge_id_equality_contract(contract: dict) -> str:
    """Export a merge-id-equality contract as stable sanitized JSON."""

    sanitized = sanitize_public_mapping(contract)
    return json.dumps(sanitized, sort_keys=True, ensure_ascii=False)


def contract_merge_id_equality_file(
    merge_json_path: Path | str,
    *,
    agent_a_merge_id: Any = None,
    agent_b_merge_id: Any = None,
) -> dict:
    """Read an exported Phase 10AS merge JSON file and create a merge-id-equality contract.
    ...
    return create_shared_merge_id_equality_contract(
        merge,
        agent_a_merge_id=agent_a_merge_id,
        agent_b_merge_id=agent_b_merge_id,
    )
