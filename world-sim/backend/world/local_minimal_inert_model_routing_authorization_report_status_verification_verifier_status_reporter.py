"""Phase 10GL - inert model-routing authorization status verification.

Consumes one caller-supplied 10GF dictionary and returns a sanitized
verification artifact. No source service, file, process, or world state is used.
"""
...
_SOURCE_SCHEMA_VERSION = "10GF.1"
_SOURCE_TYPE = "minimal_inert_model_routing_authorization_report_status_report"
_SOURCE_SCOPE = "model_routing_authorization_report_status_only"
_SOURCE_AUTHORIZATION_SCHEMA_VERSION = "10FZ.1"
_APPROVAL_STATUSES = frozenset({"operator_approved", "operator_denied"})
_SOURCE_STATUSES = frozenset({"authorized_status", "not_authorized_status"})
...
_SOURCE_MATERIAL_FIELDS = tuple(
    sorted(_SOURCE_FIELDS - {"authorization_status_report_decision_id"})
)
_VERIFICATION_MATERIAL_FIELDS = tuple(
    sorted(_OUTPUT_FIELDS - {"authorization_status_verification_decision_id"})
)
...
def _verification_decision_material(
    report: dict[str, Any],
) -> dict[str, Any]:
    return {field: report[field] for field in _VERIFICATION_MATERIAL_FIELDS}


def _validated_status_snapshot(source: object) -> dict[str, Any] | None:
    ...
    for field in _SOURCE_STRING_FIELDS:
        if type(snapshot[field]) is not str:
            return None
    status = snapshot["authorization_status_report_status"]
    ...
    if (
        ...
        or snapshot["authorization_status_report_schema_version"]
        != _SOURCE_SCHEMA_VERSION
        or snapshot["authorization_status_report_type"] != _SOURCE_TYPE
        or snapshot["authorization_status_report_scope"] != _SOURCE_SCOPE
    ...
    if snapshot["authorization_status_report_decision_id"] != expected_decision_id:
        return None
    return snapshot
...
def _verification_report(
    *,
    ...
    authorization_status_verification_status: str,
    errors: list[str],
) -> dict[str, Any]:
    safe_errors = list(errors)
    result = {
        "ok": authorization_status_verification_status
        in {
            "verified_authorized_status",
            "verified_not_authorized_status",
        },
        "authorization_status_verification_schema_version": _SCHEMA_VERSION,
        "authorization_status_verification_type": _VERIFICATION_TYPE,
        "authorization_status_verification_scope": _VERIFICATION_SCOPE,
        "authorization_status_verification_decision_id": "",
        "source_status_report_schema_version": (
            source_status_report_schema_version
        ),
    ...
    result["authorization_status_verification_decision_id"] = (
        "10GL-" + _hash_canonical(_verification_decision_material(result))[:32]
    )
    return result


def create_minimal_inert_model_routing_authorization_report_status_verification_verifier_status_report(
    status_report: dict | None,
) -> dict[str, Any]:
    """Verify one supplied 10GF status report without source access."""

    try:
        source = _validated_status_snapshot(status_report)
    except Exception:
        source = None
    if source is None:
    ...
    source_status = source["authorization_status_report_status"]
    verification_status = (
        "verified_authorized_status"
        if source_status == "authorized_status"
    ...
    return _verification_report(
        ...
        source_status_report_status=source_status,
        source_status_report_ok=source["ok"],
        ...
        authorization_status_verification_status=verification_status,
        errors=[],
    )


def export_minimal_inert_model_routing_authorization_report_status_verification_verifier_status_report(
    verification_report: dict,
) -> str:
    """Export one validated 10GL verification report as sorted JSON."""

    error = "verification report must have the exact 10GL verification report shape"
    ...
    for field in _OUTPUT_STRING_FIELDS:
        if type(report[field]) is not str:
            raise ValueError(error)
    if (
        report["authorization_status_verification_schema_version"]
        != _SCHEMA_VERSION
        or report["authorization_status_verification_type"]
        != _VERIFICATION_TYPE
        or report["authorization_status_verification_scope"]
        != _VERIFICATION_SCOPE
    ...
    status = report["authorization_status_verification_status"]
    if status == "invalid_report":
        ...
        expected_status_report_status = (
            "authorized_status"
            if status == "verified_authorized_status"
        ...
        source_is_valid = not any(
            (
                ...
                status == "verified_authorized_status"
