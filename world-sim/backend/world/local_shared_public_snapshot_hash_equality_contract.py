...
artifact from a Phase 10AS two-agent public merge.  10AY formalizes
whether two agents' known-map snapshots have identical content
fingerprints, without ever inferring private knowledge, co-presence,
awareness, trust, cooperation, conflict, communication, temporal
overlap, coordination, planning, proximity, or any kind of
relationship.

Snapshot hash data is present in the 10AS agent bundles' `snapshot_hash`
fields.  No caller-supplied arguments are needed.  If a bundle lacks
`snapshot_hash`, the contract still builds with empty/False hash fields.
...
:func:`contract_snapshot_hash_equality_file`, which only reads JSON
from a caller-supplied path.

10AY consumes 10AS only.  It never imports or calls 10AS, 10AR, 10AQ,
...
``world-sim/data``.  The only upstream symbol 10AY imports is
``sanitize_public_mapping`` from the public egress sanitizer (the same
helper 10AR, 10AS, 10AT, 10AU, 10AV, 10AW, and 10AX already import).
"""
...
_CONTRACT_SCHEMA_VERSION = "10AY.1"
_CONTRACT_TYPE = "shared_snapshot_hash_equality_contract"
_SOURCE_PHASE = "10AS"
_CLAIM_SCOPE = "shared_snapshot_hash_equality_only"
...
def _empty_contract(errors: list[str]) -> dict[str, Any]:
    return {
        ...
        "agent_a_snapshot_hash": None,
        "agent_b_snapshot_hash": None,
        "agent_a_snapshot_id": None,
        "agent_b_snapshot_id": None,
        "same_snapshot_hash": False,
        "shared_snapshot_hash": None,
...
def create_shared_snapshot_hash_equality_contract(merge: dict) -> dict:
    """Create a deterministic sanitized shared-snapshot-hash-equality contract.

    Consumes an already-built Phase 10AS two-agent public merge artifact
    and emits a thin shared-snapshot-hash-equality contract recording
    whether the two agents' known-map snapshots have identical content
    fingerprints.  Snapshot hash data is read from the agent bundles'
    `snapshot_hash` and `snapshot_id` fields.  If a bundle lacks
    `snapshot_hash`, the contract still builds with empty/False hash
    fields.  Never infers private knowledge, co-presence, awareness,
    trust, cooperation, conflict, communication, temporal overlap,
    coordination, planning, proximity, or any kind of relationship.
    ...
    # Read snapshot hash fields directly from bundles.
    a_snapshot_hash = _safe_str(sanitized_agent_a.get("snapshot_hash")) or None
    b_snapshot_hash = _safe_str(sanitized_agent_b.get("snapshot_hash")) or None
    a_snapshot_id = _safe_str(sanitized_agent_a.get("snapshot_id")) or None
    b_snapshot_id = _safe_str(sanitized_agent_b.get("snapshot_id")) or None

    same_snapshot_hash = bool(
        a_snapshot_hash and b_snapshot_hash and a_snapshot_hash == b_snapshot_hash
    )
    shared_snapshot_hash = a_snapshot_hash if same_snapshot_hash else None
    ...
    contract_material: dict[str, Any] = {
        ...
        "agent_a_snapshot_hash": a_snapshot_hash or "",
        "agent_b_snapshot_hash": b_snapshot_hash or "",
        "agent_a_snapshot_id": a_snapshot_id or "",
        "agent_b_snapshot_id": b_snapshot_id or "",
        "same_snapshot_hash": same_snapshot_hash,
        "shared_snapshot_hash": shared_snapshot_hash or "",
        "claim_scope": _CLAIM_SCOPE,
    }
    contract_id = "10AY-" + _hash_canonical(contract_material)[:32]
    ...
    return {
        ...
        "agent_a_snapshot_hash": a_snapshot_hash,
        "agent_b_snapshot_hash": b_snapshot_hash,
...
def export_shared_snapshot_hash_equality_contract(contract: dict) -> str:
    """Export a shared-snapshot-hash-equality contract as stable sanitized JSON."""

    sanitized = sanitize_public_mapping(contract)
    return json.dumps(sanitized, sort_keys=True, ensure_ascii=False)


def contract_snapshot_hash_equality_file(merge_json_path: Path | str) -> dict:
    """Read an exported Phase 10AS merge JSON file and create a shared-snapshot-hash-equality contract.
    ...
    return create_shared_snapshot_hash_equality_contract(merge)
