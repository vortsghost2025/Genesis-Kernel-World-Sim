...
Create a deterministic sanitized shared-public-event-ref contract
artifact from a Phase 10AS two-agent public merge.  10AV formalizes
which public event references two agents have each cited in their
...
Event references come from the agent bundles' ``public_event_refs``
fields when present.  The caller may also supply them directly as
optional arguments — no 10AS update is required.

10AV is the third rung in the public-observation contract stack.  It
does not infer temporal overlap, does not claim the agents were active
at the same time, does not infer co-presence or proximity, and does not
promote a shared event reference into a private relationship, trust,
cooperation, conflict, awareness, communication, or co-presence claim.
...
:func:`contract_shared_event_ref_file`, which only reads JSON from a
caller-supplied path.

10AV consumes 10AS only.  It never imports or calls 10AS, 10AR, 10AQ,
...
``world-sim/data``.  The only upstream symbol 10AV imports is
``sanitize_public_mapping`` from the public egress sanitizer (the same
helper 10AR, 10AS, 10AT, and 10AU already import).
"""
...
_CONTRACT_SCHEMA_VERSION = "10AV.1"
_CONTRACT_TYPE = "shared_public_event_ref_contract"
_SOURCE_PHASE = "10AS"
_CLAIM_SCOPE = "shared_public_event_refs_only"
...
def _empty_contract(errors: list[str]) -> dict[str, Any]:
    return {
        ...
        "shared_public_event_ref_count": 0,
...
def _resolve_event_refs(
    ...
    if caller_supplied is not None:
        return _sorted_unique_strings(caller_supplied)
    return _sorted_unique_strings(bundle.get("public_event_refs"))
...
def create_shared_public_event_ref_contract(
    merge: dict,
    ...
    and emits a thin shared-public-event-ref contract recording which
    public event references the two agents demonstrably share.  Event
    references come from the agent bundles' ``public_event_refs`` fields
    unless overridden by the optional ``agent_a_event_refs`` /
    ``agent_b_event_refs`` arguments.  Never infers private knowledge,
    co-presence, awareness, trust, cooperation, conflict, communication,
    temporal overlap, or any kind of relationship.  Never raises; always
    returns a dict.
    """
    ...
    # Resolve event refs: caller-supplied overrides take precedence,
    # otherwise fall back to the bundle's public_event_refs field.
    a_event_refs = _resolve_event_refs(sanitized_agent_a, agent_a_event_refs)
    b_event_refs = _resolve_event_refs(sanitized_agent_b, agent_b_event_refs)

    shared_event_refs = sorted(set(a_event_refs) & set(b_event_refs))
    a_only_event_refs = sorted(set(a_event_refs) - set(b_event_refs))
    b_only_event_refs = sorted(set(b_event_refs) - set(a_event_refs))
    ...
    contract_material: dict[str, Any] = {
        ...
        "shared_public_event_refs": shared_event_refs,
        "agent_a_only_event_refs": a_only_event_refs,
        "agent_b_only_event_refs": b_only_event_refs,
        "claim_scope": _CLAIM_SCOPE,
    }
    contract_id = "10AV-" + _hash_canonical(contract_material)[:32]
    ...
    return {
        ...
        "shared_public_event_refs": shared_event_refs,
        "shared_public_event_ref_count": len(shared_event_refs),
        "agent_a_only_event_refs": a_only_event_refs,
        "agent_b_only_event_refs": b_only_event_refs,
        "claim_scope": _CLAIM_SCOPE,
        "errors": [],
...
def export_shared_public_event_ref_contract(contract: dict) -> str:
    """Export a shared-public-event-ref contract as stable sanitized JSON."""

    sanitized = sanitize_public_mapping(contract)
    return json.dumps(sanitized, sort_keys=True, ensure_ascii=False)


def contract_shared_event_ref_file(merge_json_path: Path | str) -> dict:
    """Read an exported Phase 10AS merge JSON file and create a shared-public-event-ref contract.
    ...
    return create_shared_public_event_ref_contract(merge)
