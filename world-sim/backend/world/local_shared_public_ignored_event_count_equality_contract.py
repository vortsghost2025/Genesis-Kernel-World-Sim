...
Create a deterministic sanitized shared-public-ignored-event-count-equality
contract artifact from a Phase 10AS two-agent public merge.  10BF
formalizes whether two agents' public ignored event counts are identical,
without ever inferring same ignored events, temporal overlap, co-presence,
awareness, trust, cooperation, conflict, communication, coordination,
planning, proximity, or any kind of relationship.

Ignored event counts are supplied as optional caller arguments; 10BF does
not read them from 10AS bundles.  Caller-supplied ignored event counts are
validated as non-negative integers; missing, non-integer, or negative
...
:func:`contract_ignored_event_count_equality_file`, which only reads JSON
from a caller-supplied path.

10BF consumes 10AS only.  It never imports or calls 10AS, 10AR, 10AQ,
...
``world-sim/data``.  The only upstream symbol 10BF imports is
...
10BC, 10BD, and 10BE already import).
"""
...
_CONTRACT_SCHEMA_VERSION = "10BF.1"
_CONTRACT_TYPE = "shared_public_ignored_event_count_equality_contract"
_SOURCE_PHASE = "10AS"
_CLAIM_SCOPE = "shared_public_ignored_event_count_equality_only"
...
def _empty_contract(errors: list[str]) -> dict[str, Any]:
    return {
        ...
        "shared_ignored_event_count": None,
        "claim_scope": _CLAIM_SCOPE,
        "errors": errors,
    }


def _sanitize_count(raw: Any) -> int | None:
    """Sanitize a caller-supplied ignored event count.

    Returns the non-negative integer, or None if the input is not a
...
def create_shared_ignored_event_count_equality_contract(
    merge: dict,
    *,
    agent_a_ignored_event_count: Any = None,
    agent_b_ignored_event_count: Any = None,
) -> dict:
    """Create a deterministic sanitized shared-public-ignored-event-count-equality contract.
    ...
    ignored event counts.  Emits a thin ignored-event-count-equality
    contract recording whether the two agents' supplied ignored event
    counts are mechanically equal.  Caller-supplied ignored event counts
    are validated as non-negative integers; missing, non-integer, or
    negative values are treated as ``None``.  Missing or None ignored
    event count fields produce a contract with ok=True, count fields set
    to None, and all equality booleans set to False.  Never infers same
    ignored events, temporal overlap, co-presence, awareness, trust,
    ...
    # --- Caller-supplied ignored event counts ----------------------------
    # These are NOT read from 10AS bundles.  They are caller-supplied
    # optional arguments.  Validate as non-negative integers; missing,
    # non-integer, or negative values become None.

    a_count = _sanitize_count(agent_a_ignored_event_count)
    b_count = _sanitize_count(agent_b_ignored_event_count)

    # --- Count comparison -------------------------------------------------
    # Pure integer equality.  No semantic interpretation of count meaning.
    # No ordering, no sequence inference, no temporal inference.

    same_ignored_event_count = bool(
        a_count is not None
        and b_count is not None
        and a_count == b_count
    )
    shared_ignored_event_count = a_count if same_ignored_event_count else None
    ...
    contract_material: dict[str, Any] = {
        ...
        "agent_a_ignored_event_count": a_count if a_count is not None else "",
        "agent_b_ignored_event_count": b_count if b_count is not None else "",
        "same_ignored_event_count": same_ignored_event_count,
        "shared_ignored_event_count": (
            shared_ignored_event_count
            if shared_ignored_event_count is not None
            else ""
        ),
    }
    contract_id = "10BF-" + _hash_canonical(contract_material)[:32]
    ...
    return {
        ...
        "agent_a_ignored_event_count": a_count,
        "agent_b_ignored_event_count": b_count,
        "same_ignored_event_count": same_ignored_event_count,
        "shared_ignored_event_count": shared_ignored_event_count,
        "claim_scope": _CLAIM_SCOPE,
        "errors": [],
    }


def export_shared_ignored_event_count_equality_contract(contract: dict) -> str:
    """Export an ignored-event-count-equality contract as stable sanitized JSON."""

    sanitized = sanitize_public_mapping(contract)
    return json.dumps(sanitized, sort_keys=True, ensure_ascii=False)


def contract_ignored_event_count_equality_file(
    merge_json_path: Path | str,
    *,
    agent_a_ignored_event_count: Any = None,
    agent_b_ignored_event_count: Any = None,
) -> dict:
    """Read an exported Phase 10AS merge JSON file and create an ignored-event-count-equality contract.
    ...
    return create_shared_ignored_event_count_equality_contract(
        merge,
        agent_a_ignored_event_count=agent_a_ignored_event_count,
        agent_b_ignored_event_count=agent_b_ignored_event_count,
    )
