"""Phase 10GX - inert model-routing authorization meta-meta-verification.

Consumes one caller-supplied 10GR dictionary and returns a sanitized
meta-meta-verification artifact. No source service, file, process, or world
state is used.
"""
...
_SCHEMA_VERSION = "10GX.1"
_META_META_VERIFICATION_TYPE = (
    "minimal_inert_model_routing_authorization_report_status_verification_"
    "verifier_status_verification_verifier_status_verification_verifier_"
    "status_report"
)
_META_META_VERIFICATION_SCOPE = (
    "model_routing_authorization_report_status_verification_verifier_status_"
    "verification_verifier_status_verification_only"
)

_SOURCE_SCHEMA_VERSION = "10GR.1"
_SOURCE_TYPE = (
    "minimal_inert_model_routing_authorization_report_status_verification_"
...
_SOURCE_VERIFICATION_SCHEMA_VERSION = "10GL.1"
...
_SOURCE_META_VERIFICATION_STATUSES = frozenset(
    {
...
_META_META_MATERIAL_FIELDS = tuple(
    sorted(
        _OUTPUT_FIELDS
        - {"authorization_status_meta_meta_verification_decision_id"}
    )
...
def _is_safe_text(value: object) -> bool:
    ...
    return not any(
        (
            ...
            and (
                value.count(":") != 1
                or not value.startswith(("lane:", "provider:", "model:"))
            ),
            "sk-" in lowered,
            "pk-" in lowered,
            lowered.startswith("hf_"),
            lowered.startswith("xapp-"),
            any(
...
def _source_decision_material_from_output(
    report: dict[str, Any],
) -> dict[str, Any]:
    return {
        "ok": report["source_meta_verification_ok"],
        "authorization_status_meta_verification_schema_version": report[
            "source_meta_verification_schema_version"
        ],
        "authorization_status_meta_verification_type": _SOURCE_TYPE,
        "authorization_status_meta_verification_scope": _SOURCE_SCOPE,
        "source_verification_schema_version": report[
            "source_verification_schema_version"
        ],
        "source_verification_decision_id": report[
            "source_verification_decision_id"
        ],
        "source_verification_status": report["source_verification_status"],
        "source_verification_ok": report["source_verification_ok"],
        "source_status_report_schema_version": report[
            "source_status_report_schema_version"
        ],
        "source_status_report_decision_id": report[
            "source_status_report_decision_id"
        ],
        "source_status_report_status": report["source_status_report_status"],
        "source_status_report_ok": report["source_status_report_ok"],
        "source_authorization_schema_version": report[
            "source_authorization_schema_version"
        ],
        "source_authorization_decision_id": report[
            "source_authorization_decision_id"
        ],
        "source_authorization_status": report["source_authorization_status"],
        "source_authorization_ok": report["source_authorization_ok"],
        "source_provenance_decision_id": report[
            "source_provenance_decision_id"
        ],
        "source_policy_decision_id": report["source_policy_decision_id"],
        ...
        "authorized_lane": report["authorized_lane"],
        "provider_id": report["provider_id"],
        "provider_name": report["provider_name"],
        "pinned_model_id": report["pinned_model_id"],
        "pinned_model_revision": report["pinned_model_revision"],
        "authorization_status_meta_verification_status": report[
            "source_meta_verification_status"
        ],
        "executed": report["executed"],
        "runtime_allowed": report["runtime_allowed"],
        "daemon_allowed": report["daemon_allowed"],
        "scheduler_allowed": report["scheduler_allowed"],
        "network_allowed": report["network_allowed"],
        "world_sim_data_accessed": report["world_sim_data_accessed"],
        "gate7_activity_allowed": report["gate7_activity_allowed"],
        "claim_boundary": _SOURCE_CLAIM_BOUNDARY,
        "errors": [],
    }


def _meta_meta_decision_material(
    report: dict[str, Any],
) -> dict[str, Any]:
    return {field: report[field] for field in _META_META_MATERIAL_FIELDS}


def _validated_meta_verification_snapshot(
    source: object,
) -> dict[str, Any] | None:
    ...
    for field in _SOURCE_STRING_FIELDS:
        if type(snapshot[field]) is not str:
            return None
    meta_status = snapshot["authorization_status_meta_verification_status"]
    verification_status = snapshot["source_verification_status"]
    ...
    authorized = meta_status == "verified_authorized_verification_status"
    if (
        snapshot["ok"] is not True
        or snapshot["source_verification_ok"] is not True
        or snapshot["source_status_report_ok"] is not True
        or snapshot["source_authorization_ok"] is not True
        or snapshot["authorization_status_meta_verification_schema_version"]
        != _SOURCE_SCHEMA_VERSION
        or snapshot["authorization_status_meta_verification_type"]
        != _SOURCE_TYPE
        or snapshot["authorization_status_meta_verification_scope"]
        != _SOURCE_SCOPE
        or snapshot["source_verification_schema_version"]
        != _SOURCE_VERIFICATION_SCHEMA_VERSION
        ...
        or meta_status not in _SOURCE_META_VERIFICATION_STATUSES
        ...
        or not _is_decision_id(
            snapshot["authorization_status_meta_verification_decision_id"],
            "10GR-",
        )
        or not _is_decision_id(
            snapshot["source_verification_decision_id"],
            "10GL-",
        )
        or not _is_decision_id(
...
def _meta_meta_verification_report(
    *,
    source_meta_verification_schema_version: str = "",
    source_meta_verification_decision_id: str = "",
    source_meta_verification_status: str = "",
    source_meta_verification_ok: bool = False,
    ...
    authorization_status_meta_meta_verification_status: str,
    errors: list[str],
) -> dict[str, Any]:
    safe_errors = list(errors)
    result = {
        "ok": authorization_status_meta_meta_verification_status
        in {
        ...
        "source_meta_verification_status": source_meta_verification_status,
        "source_meta_verification_ok": source_meta_verification_ok,
    ...
    result["authorization_status_meta_meta_verification_decision_id"] = (
        "10GX-" + _hash_canonical(_meta_meta_decision_material(result))[:32]
    )
    return result


def create_minimal_inert_model_routing_authorization_report_status_verification_verifier_status_verification_verifier_status_verification_verifier_status_report(
    meta_verification_report: dict | None,
) -> dict[str, Any]:
    """Meta-meta-verify one supplied 10GR report without source access."""

    try:
        source = _validated_meta_verification_snapshot(meta_verification_report)
    except Exception:
        source = None
    if source is None:
        return _meta_meta_verification_report(
            authorization_status_meta_meta_verification_status=(
    ...
    source_status = source["authorization_status_meta_verification_status"]
    meta_meta_status = (
        "verified_authorized_meta_verification_status"
        if source_status == "verified_authorized_verification_status"
        else "verified_not_authorized_meta_verification_status"
    )
    return _meta_meta_verification_report(
        source_meta_verification_schema_version=source[
            "authorization_status_meta_verification_schema_version"
        ],
        source_meta_verification_decision_id=source[
            "authorization_status_meta_verification_decision_id"
        ],
        source_meta_verification_status=source_status,
        source_meta_verification_ok=source["ok"],
        ...
        source_verification_ok=source["source_verification_ok"],
        ...
        authorization_status_meta_meta_verification_status=meta_meta_status,
        errors=[],
    )


def export_minimal_inert_model_routing_authorization_report_status_verification_verifier_status_verification_verifier_status_verification_verifier_status_report(
    meta_meta_verification_report: dict,
) -> str:
    """Export one validated 10GX meta-meta-verification report as JSON."""

    error = "meta-meta-verification report must have the exact 10GX meta-meta-verification report shape"
    if type(meta_meta_verification_report) is not dict:
        raise ValueError(error)
    report = dict(meta_meta_verification_report)
    ...
    for field in _OUTPUT_STRING_FIELDS:
        if type(report[field]) is not str:
            raise ValueError(error)
    if (
        report["authorization_status_meta_meta_verification_schema_version"]
        != _SCHEMA_VERSION
        or report["authorization_status_meta_meta_verification_type"]
        != _META_META_VERIFICATION_TYPE
        or report["authorization_status_meta_meta_verification_scope"]
        != _META_META_VERIFICATION_SCOPE
        ...
        or type(report["source_meta_verification_ok"]) is not bool
    ...
    status = report["authorization_status_meta_meta_verification_status"]
    if status == "invalid_meta_verification_report":
        ...
        source_is_valid = not any(
            (
                ...
                report["source_meta_verification_ok"] is not False,
        ...
        expected_errors = []
        authorized = status == "verified_authorized_meta_verification_status"
        expected_meta_status = (
            "verified_authorized_verification_status"
            if authorized
        ...
        source_is_valid = not any(
            (
                report["source_meta_verification_schema_version"]
                != _SOURCE_SCHEMA_VERSION,
                not _is_decision_id(
                    report["source_meta_verification_decision_id"],
                    "10GR-",
                ),
                report["source_meta_verification_decision_id"]
                != "10GR-"
                + _hash_canonical(
                    _source_decision_material_from_output(report)
                )[:32],
                report["source_meta_verification_status"]
                != expected_meta_status,
                report["source_meta_verification_ok"] is not True,
    ...
    expected_decision_id = "10GX-" + _hash_canonical(
        _meta_meta_decision_material(report)
    )[:32]
    if (
        report["authorization_status_meta_meta_verification_decision_id"]
        != expected_decision_id
