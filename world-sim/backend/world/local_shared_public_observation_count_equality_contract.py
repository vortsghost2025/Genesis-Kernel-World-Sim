...
Create a deterministic sanitized shared-public-observation-count-equality
contract artifact from a Phase 10AS two-agent public merge.  10BC
formalizes whether two agents' public observation counts are identical,
without ever inferring same observations, temporal overlap, co-presence,
...
not read them from 10AS bundles.  Caller-supplied observation counts are
...
:func:`contract_observation_count_equality_file`, which only reads JSON
from a caller-supplied path.

10BC consumes 10AS only.  It never imports or calls 10AS, 10AR, 10AQ,
...
``world-sim/data``.  The only upstream symbol 10BC imports is
``sanitize_public_mapping`` from the public egress sanitizer (the same
helper 10AR, 10AS, 10AT, 10AU, 10AV, 10AW, 10AX, 10AY, 10AZ, and 10BB
already import).
"""
...
_CONTRACT_SCHEMA_VERSION = "10BC.1"
_CONTRACT_TYPE = "shared_public_observation_count_equality_contract"
_SOURCE_PHASE = "10AS"
_CLAIM_SCOPE = "shared_public_observation_count_equality_only"
...
def _sanitize_count(raw: Any) -> int | None:
    """Sanitize a caller-supplied observation count.
...
def create_shared_observation_count_equality_contract(
    merge: dict,
    *,
    agent_a_observation_count: Any = None,
    agent_b_observation_count: Any = None,
) -> dict:
    """Create a deterministic sanitized shared-public-observation-count-equality contract.
    ...
    observation counts.  Emits a thin observation-count-equality contract
    recording whether the two agents' supplied observation counts are
    mechanically equal.  Caller-supplied observation counts are validated
    as non-negative integers; missing, non-integer, or negative values
    are treated as ``None``.  Missing or None observation count fields
    produce a contract with ok=True, count fields set to None, and all
    equality booleans set to False.  Never infers same observations,
    temporal overlap, co-presence, awareness, trust, cooperation,
    ...
    # --- Caller-supplied observation counts ------------------------------
    ...
    a_count = _sanitize_count(agent_a_observation_count)
    b_count = _sanitize_count(agent_b_observation_count)
    ...
    shared_observation_count = a_count if same_observation_count else None
    ...
    contract_material: dict[str, Any] = {
        ...
        "agent_a_observation_count": a_count if a_count is not None else "",
        "agent_b_observation_count": b_count if b_count is not None else "",
        "same_observation_count": same_observation_count,
        "shared_observation_count": (
            shared_observation_count
            if shared_observation_count is not None
            else ""
        ),
    }
    contract_id = "10BC-" + _hash_canonical(contract_material)[:32]
    ...
    return {
        ...
        "agent_a_observation_count": a_count,
        "agent_b_observation_count": b_count,
        "same_observation_count": same_observation_count,
...
def export_shared_observation_count_equality_contract(contract: dict) -> str:
    """Export an observation-count-equality contract as stable sanitized JSON."""

    sanitized = sanitize_public_mapping(contract)
    return json.dumps(sanitized, sort_keys=True, ensure_ascii=False)


def contract_observation_count_equality_file(
    merge_json_path: Path | str,
    *,
    agent_a_observation_count: Any = None,
    agent_b_observation_count: Any = None,
) -> dict:
    """Read an exported Phase 10AS merge JSON file and create an observation-count-equality contract.
    ...
    return create_shared_observation_count_equality_contract(
        merge,
        agent_a_observation_count=agent_a_observation_count,
        agent_b_observation_count=agent_b_observation_count,
    )