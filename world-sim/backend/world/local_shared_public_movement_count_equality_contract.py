...
Create a deterministic sanitized shared-public-movement-count-equality
contract artifact from a Phase 10AS two-agent public merge.  10BD
formalizes whether two agents' public movement counts are identical,
without ever inferring same movement, temporal overlap, co-presence,
...
not read them from 10AS bundles.  Caller-supplied movement counts are
...
:func:`contract_movement_count_equality_file`, which only reads JSON
from a caller-supplied path.

10BD consumes 10AS only.  It never imports or calls 10AS, 10AR, 10AQ,
...
``world-sim/data``.  The only upstream symbol 10BD imports is
...
_CONTRACT_SCHEMA_VERSION = "10BD.1"
_CONTRACT_TYPE = "shared_public_movement_count_equality_contract"
_SOURCE_PHASE = "10AS"
_CLAIM_SCOPE = "shared_public_movement_count_equality_only"
...
def _sanitize_count(raw: Any) -> int | None:
    """Sanitize a caller-supplied movement count.
...
def create_shared_movement_count_equality_contract(
    merge: dict,
    *,
    agent_a_movement_count: Any = None,
    agent_b_movement_count: Any = None,
) -> dict:
    """Create a deterministic sanitized shared-public-movement-count-equality contract.
    ...
    movement counts.  Emits a thin movement-count-equality contract
    recording whether the two agents' supplied movement counts are
    mechanically equal.  Caller-supplied movement counts are validated
    as non-negative integers; missing, non-integer, or negative values
    are treated as ``None``.  Missing or None movement count fields
    produce a contract with ok=True, count fields set to None, and all
    equality booleans set to False.  Never infers same movement,
    ...
    # --- Caller-supplied movement counts --------------------------------
    ...
    a_count = _sanitize_count(agent_a_movement_count)
    b_count = _sanitize_count(agent_b_movement_count)

    # --- Count comparison -------------------------------------------------
    # Pure integer equality.  No semantic interpretation of count meaning.
    # No ordering, no sequence inference, no temporal inference.

    same_movement_count = bool(
        a_count is not None
        and b_count is not None
        and a_count == b_count
    )
    shared_movement_count = a_count if same_movement_count else None
    ...
    contract_material: dict[str, Any] = {
        ...
        "agent_a_movement_count": a_count if a_count is not None else "",
        "agent_b_movement_count": b_count if b_count is not None else "",
        "same_movement_count": same_movement_count,
        "shared_movement_count": (
            shared_movement_count
            if shared_movement_count is not None
            else ""
        ),
    }
    contract_id = "10BD-" + _hash_canonical(contract_material)[:32]
    ...
    return {
        ...
        "agent_a_movement_count": a_count,
        "agent_b_movement_count": b_count,
        "same_movement_count": same_movement_count,
        "shared_movement_count": shared_movement_count,
        "claim_scope": _CLAIM_SCOPE,
        "errors": [],
    }


def export_shared_movement_count_equality_contract(contract: dict) -> str:
    """Export a movement-count-equality contract as stable sanitized JSON."""

    sanitized = sanitize_public_mapping(contract)
    return json.dumps(sanitized, sort_keys=True, ensure_ascii=False)


def contract_movement_count_equality_file(
    merge_json_path: Path | str,
    *,
    agent_a_movement_count: Any = None,
    agent_b_movement_count: Any = None,
) -> dict:
    """Read an exported Phase 10AS merge JSON file and create a movement-count-equality contract.
    ...
    return create_shared_movement_count_equality_contract(
        merge,
        agent_a_movement_count=agent_a_movement_count,
        agent_b_movement_count=agent_b_movement_count,
    )
