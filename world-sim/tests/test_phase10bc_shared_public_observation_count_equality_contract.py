"""Phase 10BC - shared public observation count equality contract proof.

These tests prove that a Phase 10AS two-agent public merge artifact can
be wrapped in a deterministic, sanitized shared-public-observation-count-
equality contract that formalizes whether two agents' public observation
counts are identical, without ever inferring same observations, temporal
overlap, co-presence, awareness, trust, cooperation, conflict,
communication, coordination, planning, proximity, or any kind of
relationship.

Observation counts are supplied as optional caller arguments; 10BC does
not read them from 10AS bundles.  Missing, non-integer, or negative
observation counts are treated as ``None``.
...
10BC may say:
    ...
    "Both agents report the same public observation_count value."
...
from backend.world.local_shared_public_observation_count_equality_contract import (  # noqa: E402
    contract_observation_count_equality_file,
    create_shared_observation_count_equality_contract,
    export_shared_observation_count_equality_contract,
)
...
# fabricate valid 10AS merge inputs.  The 10BC MODULE under test
# (local_shared_public_observation_count_equality_contract.py) does not
...
# test_phase10ba_shared_public_tick_label_contract.py, and
...
# ---------------------------------------------------------------------------
# Fixtures: build real 10AS merges to feed 10BC
# ---------------------------------------------------------------------------
...
class TestCreateSharedObservationCountEqualityContract:
    """Tests 1-21 from the 10BC spec test plan."""

    # 1. Happy path: both bundles carry equal observation_count
    def test_happy_path_equal_counts(self):
        merge = _build_merge()
        contract = create_shared_observation_count_equality_contract(
            merge,
            agent_a_observation_count=3,
            agent_b_observation_count=3,
        )
        assert contract["ok"] is True
        assert contract["contract_schema_version"] == "10BC.1"
        assert contract["contract_type"] == "shared_public_observation_count_equality_contract"
        assert contract["contract_id"].startswith("10BC-")
        assert contract["claim_scope"] == "shared_public_observation_count_equality_only"
        assert contract["errors"] == []
        assert contract["same_observation_count"] is True
        assert contract["shared_observation_count"] == 3

    # 2. Different observation counts
    def test_different_observation_counts(self):
        merge = _build_merge()
        contract = create_shared_observation_count_equality_contract(
            merge,
            agent_a_observation_count=3,
        ...
        assert contract["ok"] is True
        assert contract["same_observation_count"] is False
        assert contract["shared_observation_count"] is None
        assert contract["agent_a_observation_count"] == 3
        assert contract["agent_b_observation_count"] == 5

    # 3. One bundle missing observation_count
    def test_one_bundle_missing_observation_count(self):
        merge = _build_merge()
        contract = create_shared_observation_count_equality_contract(
            merge,
            agent_a_observation_count=None,
            agent_b_observation_count=3,
        )
        assert contract["ok"] is True
        assert contract["agent_a_observation_count"] is None
        assert contract["agent_b_observation_count"] == 3
        assert contract["same_observation_count"] is False
        assert contract["shared_observation_count"] is None

    # 4. Both bundles missing observation_count
    def test_both_bundles_missing_observation_count(self):
        merge = _build_merge()
        contract = create_shared_observation_count_equality_contract(
            merge,
            agent_a_observation_count=None,
            agent_b_observation_count=None,
        )
        assert contract["ok"] is True
        assert contract["agent_a_observation_count"] is None
        assert contract["agent_b_observation_count"] is None
        assert contract["same_observation_count"] is False
        assert contract["shared_observation_count"] is None
    ...
    def test_contract_id_deterministic(self):
        merge = _build_merge()
        c1 = create_shared_observation_count_equality_contract(
            merge,
            agent_a_observation_count=3,
            agent_b_observation_count=3,
        )
        c2 = create_shared_observation_count_equality_contract(
            merge,
            agent_a_observation_count=3,
            agent_b_observation_count=3,
        )
    ...
    def test_contract_id_changes_with_count(self):
        ...
        assert c1["same_observation_count"] != c2["same_observation_count"]
        assert c1["contract_id"] != c2["contract_id"]
    ...
    def test_structural_failures(self):
        # Non-dict
        c = create_shared_observation_count_equality_contract("not a dict")
        ...
        bad_merge = {"ok": False, "merge_type": "two_agent_public_merge",
                     ...
                     "agent_b": {}}
        c = create_shared_observation_count_equality_contract(bad_merge)
        assert c["ok"] is False
    ...
    # 10. Type validation: observation_count not an integer
    def test_type_validation_non_integer_observation_count(self):
        merge = _build_merge()
        contract = create_shared_observation_count_equality_contract(
            merge,
            agent_a_observation_count="three",
            agent_b_observation_count=3,
        )
        assert contract["ok"] is True
        assert contract["agent_a_observation_count"] is None
    ...
    def test_private_markers_redacted(self):
        ...
        exported = export_shared_observation_count_equality_contract(contract)
        assert "/home/user/secret" not in exported

    # 12. Graceful handling of missing/None observation counts
    def test_graceful_missing_observation_counts(self):
        merge = _build_merge()
        contract = create_shared_observation_count_equality_contract(
            merge,
            agent_a_observation_count=None,
    ...
    def test_export_stable_json(self):
        ...
        exported2 = export_shared_observation_count_equality_contract(parsed)
        assert exported == exported2

    # 14. contract_observation_count_equality_file reads and builds
    def test_contract_observation_count_equality_file(self):
        merge = _build_merge()
        with tempfile.TemporaryDirectory() as tmpdir:
            path = Path(tmpdir) / "merge.json"
            path.write_text(json.dumps(merge), encoding="utf-8")
            result = contract_observation_count_equality_file(
                path,
                agent_a_observation_count=3,
                agent_b_observation_count=3,
            )
        assert result["ok"] is True
        assert result["agent_a_observation_count"] == 3
        assert result["agent_b_observation_count"] == 3
    ...
    def test_all_public_functions_exercised(self):
        ...
        exported = export_shared_observation_count_equality_contract(c1)
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            path = Path(tmpdir) / "merge.json"
            path.write_text(json.dumps(merge), encoding="utf-8")
            c2 = contract_observation_count_equality_file(
                path,
                agent_a_observation_count=3,
                agent_b_observation_count=3,
            )
        assert c2["ok"] is True
        assert c2["same_observation_count"] is True
    ...
    def test_module_has_no_forbidden_imports(self):
        module_path = (
            ...
            / "backend"
            / "world"
            / "local_shared_public_observation_count_equality_contract.py"
        )
        ...
        for token in forbidden:
            assert token not in prose, (
                "forbidden token in 10BC module source: " + token
            )
    ...
    def test_module_does_not_call_upstream_creators(self):
        ...
        for creator in creators:
            assert creator not in prose, (
                "10BC module must not call upstream creator: " + creator
            )
...
class TestObservationCountBoundaries:
    """Additional boundary cases for 10BC."""

    # 19. Negative observation_count treated as None
    def test_negative_observation_count_treated_as_none(self):
        merge = _build_merge()
        contract = create_shared_observation_count_equality_contract(
            merge,
            agent_a_observation_count=-1,
            agent_b_observation_count=3,
        )
        assert contract["ok"] is True
        assert contract["agent_a_observation_count"] is None
    ...
    # 20. Zero observation_count handled correctly
    def test_zero_observation_count_handled(self):
        merge = _build_merge()
        contract = create_shared_observation_count_equality_contract(
            merge,
            agent_a_observation_count=0,
            agent_b_observation_count=0,
        )
        assert contract["ok"] is True
        assert contract["agent_a_observation_count"] == 0
        assert contract["agent_b_observation_count"] == 0
        assert contract["same_observation_count"] is True
        assert contract["shared_observation_count"] == 0
...
class TestObservationCountScalarOnly:
    """Additional scalar-only boundary cases for 10BC."""

    # 10BC is scalar-only: no lists, no dedup, no set algebra.
    ...
    def test_no_list_fields_in_output(self):
        ...
        for key, value in contract.items():
            ...
            assert not isinstance(value, list), (
                "10BC is scalar-only; field " + key + " must not be a list"
            )
    ...
    def test_counts_not_present_beyond_count_fields(self):
        ...
        forbidden_fields = [
            "agent_a_observation_count_count",
            "agent_b_observation_count_count",
            "shared_observation_count_count",
        ]
        for field in forbidden_fields:
            assert field not in contract, (
                "10BC is scalar-only; count field " + field + " must not exist"
            )