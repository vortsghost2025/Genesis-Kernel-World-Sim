"""Phase 10BH - shared public first event id equality contract proof.

These tests prove that a Phase 10AS two-agent public merge artifact can
be wrapped in a deterministic, sanitized shared-public-first-event-id-
equality contract that formalizes whether two agents' public first event
IDs are identical, without ever inferring same event, same time, same
sequence, same interaction, or same relationship.

First event IDs are supplied as optional caller arguments; 10BH does
not read them from 10AS bundles.  Missing, non-string, or empty-after-
sanitize first event IDs are treated as ``None``.
...
10BH may say:
    ...
    "Both agents report the same public first_event_id value."
...
from backend.world.local_shared_public_first_event_id_equality_contract import (  # noqa: E402
    contract_first_event_id_equality_file,
    create_shared_first_event_id_equality_contract,
    export_shared_first_event_id_equality_contract,
)
...
# fabricate valid 10AS merge inputs.  The 10BH MODULE under test
# (local_shared_public_first_event_id_equality_contract.py) does not
...
# test_phase10bf_shared_public_ignored_event_count_equality_contract.py, and
# test_phase10bg_shared_public_last_event_id_equality_contract.py.
...
# ---------------------------------------------------------------------------
# Fixtures: build real 10AS merges to feed 10BH
# ---------------------------------------------------------------------------
...
class TestCreateSharedFirstEventIdEqualityContract:
    """Tests 1-21 from the 10BH spec test plan."""

    # 1. Happy path: both caller-supplied first event IDs equal
    def test_happy_path_equal_ids(self):
        merge = _build_merge()
        contract = create_shared_first_event_id_equality_contract(
            merge,
            agent_a_first_event_id="ev-adam",
            agent_b_first_event_id="ev-adam",
        )
        assert contract["ok"] is True
        assert contract["contract_schema_version"] == "10BH.1"
        assert contract["contract_type"] == "shared_public_first_event_id_equality_contract"
        assert contract["contract_id"].startswith("10BH-")
        assert contract["claim_scope"] == "shared_public_first_event_id_equality_only"
        assert contract["errors"] == []
        assert contract["same_first_event_id"] is True
        assert contract["shared_first_event_id"] == "ev-adam"

    # 2. Different caller-supplied first event IDs
    def test_different_first_event_ids(self):
        merge = _build_merge()
        contract = create_shared_first_event_id_equality_contract(
            merge,
            agent_a_first_event_id="ev-adam",
            agent_b_first_event_id="ev-eve",
        )
        assert contract["ok"] is True
        assert contract["same_first_event_id"] is False
        assert contract["shared_first_event_id"] is None
        assert contract["agent_a_first_event_id"] == "ev-adam"
        assert contract["agent_b_first_event_id"] == "ev-eve"

    # 3. One caller-supplied first event ID missing/None
    def test_one_bundle_missing_first_event_id(self):
        merge = _build_merge()
        contract = create_shared_first_event_id_equality_contract(
            merge,
            agent_a_first_event_id=None,
            agent_b_first_event_id="ev-eve",
        )
        assert contract["ok"] is True
        assert contract["agent_a_first_event_id"] is None
        assert contract["agent_b_first_event_id"] == "ev-eve"
        assert contract["same_first_event_id"] is False
        assert contract["shared_first_event_id"] is None

    # 4. Both caller-supplied first event IDs missing/None
    def test_both_bundles_missing_first_event_id(self):
        merge = _build_merge()
        contract = create_shared_first_event_id_equality_contract(
            merge,
            agent_a_first_event_id=None,
            agent_b_first_event_id=None,
        )
        assert contract["ok"] is True
        assert contract["agent_a_first_event_id"] is None
        assert contract["agent_b_first_event_id"] is None
        assert contract["same_first_event_id"] is False
        assert contract["shared_first_event_id"] is None
    ...
    def test_contract_id_changes_with_id(self):
        ...
        assert c1["same_first_event_id"] != c2["same_first_event_id"]
        assert c1["contract_id"] != c2["contract_id"]
    ...
    def test_structural_failures(self):
        # Non-dict
        c = create_shared_first_event_id_equality_contract("not a dict")
        ...
        bad_merge = {"ok": False, "merge_type": "two_agent_public_merge",
                     ...
                     "agent_b": {}}
        c = create_shared_first_event_id_equality_contract(bad_merge)
        assert c["ok"] is False
    ...
    # 10. Type validation: first_event_id not a string or empty after sanitize
    def test_type_validation_non_string_first_event_id(self):
        merge = _build_merge()
        contract = create_shared_first_event_id_equality_contract(
            merge,
            agent_a_first_event_id=123,
            agent_b_first_event_id="ev-eve",
        )
        assert contract["ok"] is True
        assert contract["agent_a_first_event_id"] is None
    ...
    def test_private_markers_redacted(self):
        ...
        contract = create_shared_first_event_id_equality_contract(
            merge,
            agent_a_first_event_id="/home/user/secret",
            agent_b_first_event_id="/home/user/secret",
        )
        ...
        assert contract["same_first_event_id"] is False
        assert contract["shared_first_event_id"] is None
        exported = export_shared_first_event_id_equality_contract(contract)
        assert "/home/user/secret" not in exported

    # 12. Graceful handling of missing/None first event IDs
    def test_graceful_missing_first_event_ids(self):
        merge = _build_merge()
        contract = create_shared_first_event_id_equality_contract(
            merge,
            agent_a_first_event_id=None,
    ...
    def test_export_stable_json(self):
        ...
        exported2 = export_shared_first_event_id_equality_contract(parsed)
        assert exported == exported2

    # 14. contract_first_event_id_equality_file reads and builds
    def test_contract_first_event_id_equality_file(self):
        merge = _build_merge()
        with tempfile.TemporaryDirectory() as tmpdir:
            path = Path(tmpdir) / "merge.json"
            path.write_text(json.dumps(merge), encoding="utf-8")
            result = contract_first_event_id_equality_file(
                path,
                agent_a_first_event_id="ev-adam",
                agent_b_first_event_id="ev-adam",
            )
        assert result["ok"] is True
        assert result["agent_a_first_event_id"] == "ev-adam"
        assert result["agent_b_first_event_id"] == "ev-adam"
    ...
    def test_all_public_functions_exercised(self):
        ...
        exported = export_shared_first_event_id_equality_contract(c1)
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            path = Path(tmpdir) / "merge.json"
            path.write_text(json.dumps(merge), encoding="utf-8")
            c2 = contract_first_event_id_equality_file(
                path,
                agent_a_first_event_id="ev-adam",
                agent_b_first_event_id="ev-adam",
            )
        assert c2["ok"] is True
        assert c2["same_first_event_id"] is True
    ...
    def test_module_has_no_forbidden_imports(self):
        module_path = (
            ...
            / "backend"
            / "world"
            / "local_shared_public_first_event_id_equality_contract.py"
        )
        ...
        for token in forbidden:
            assert token not in prose, (
                "forbidden token in 10BH module source: " + token
            )
    ...
    def test_module_does_not_call_upstream_creators(self):
        ...
        for creator in creators:
            assert creator not in prose, (
                "10BH module must not call upstream creator: " + creator
            )
...
class TestFirstEventIdBoundaries:
    """Additional boundary cases for 10BH."""

    # 19. Boundary: private-marker-like first event ID is sanitized/redacted and never leaks
    def test_private_marker_like_first_event_id_never_leaks(self):
        merge = _build_merge()
        contract = create_shared_first_event_id_equality_contract(
            merge,
            agent_a_first_event_id="/home/user/secret",
    ...
    def test_empty_string_first_event_id_treated_as_none(self):
        merge = _build_merge()
        contract = create_shared_first_event_id_equality_contract(
            merge,
            agent_a_first_event_id="",
    ...
    def test_contract_id_preserves_ab_orientation(self):
        ...
        c1 = create_shared_first_event_id_equality_contract(
            merge,
            agent_a_first_event_id="ev-1",
            agent_b_first_event_id="ev-2",
        )
        ...
        c2 = create_shared_first_event_id_equality_contract(
            swap_merge,
            agent_a_first_event_id="ev-2",
            agent_b_first_event_id="ev-1",
        )
...
class TestFirstEventIdScalarOnly:
    """Additional scalar-only boundary cases for 10BH."""

    # 10BH is scalar-only: no lists, no dedup, no set algebra.
    ...
    def test_no_list_fields_in_output(self):
        ...
        for key, value in contract.items():
            ...
            assert not isinstance(value, list), (
                "10BH is scalar-only; field " + key + " must not be a list"
            )
    ...
    def test_event_ids_not_present_beyond_id_fields(self):
        ...
        forbidden_fields = [
            "agent_a_first_event_id_count",
            "agent_b_first_event_id_count",
            "shared_first_event_id_count",
        ]
