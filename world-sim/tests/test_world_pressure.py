...
module (ledger split, most-held food consumption choice, the precise
per-ledger cap rule with frozen rejection strings), runtime wiring
(per-heartbeat consumption, gather cap, famished build gate, fail-closed
behavior, observation keys), prompt projection, heartbeat outcome
persistence (census contract), and exporter pressure block.
"""
...
# ---------------------------------------------------------------------------
# Pure physics: the precise per-ledger cap rule
# ---------------------------------------------------------------------------


class TestGatherCapRule:
    # under-cap: fill up to the cap
    def test_under_cap_food_allowed(self):
        assert gather_rejection("wild_berries", {"wild_berries": 19}, 19, 0) is None

    def test_under_cap_food_rejected_at_cap(self):
        # started at cap (20), did not eat yet this tick -> full
        assert (gather_rejection("wild_berries", {"wild_berries": 20}, 20, 0)
                == GATHER_REJECT_FOOD)

    def test_food_cap_refill_after_eating(self):
        # started 20, ate 1 -> now 19, one refill allowed
        assert gather_rejection("wild_berries", {"wild_berries": 19}, 20, 0) is None

    def test_goods_fill_to_cap(self):
        assert gather_rejection("stone", {"stone": 19}, 0, 19) is None
        assert (gather_rejection("stone", {"stone": 20}, 0, 20)
                == GATHER_REJECT_GOODS)

    # over-cap grandfathering
    def test_over_cap_goods_never_grow(self):
        assert (gather_rejection("stone", {"stone": 64}, 0, 65)
                == GATHER_REJECT_GOODS)

    def test_over_cap_food_net_zero_replacement_allowed(self):
        # started 45 (over cap), ate 1 -> 44; replacing that 1 is allowed
        assert gather_rejection("wild_berries", {"wild_berries": 44}, 45, 0) is None

    def test_over_cap_food_cannot_grow_beyond_start(self):
        # started 45, did not eat (impossible in-loop, but rule must hold)
        assert (gather_rejection("wild_berries", {"wild_berries": 45}, 45, 0)
                == GATHER_REJECT_FOOD)
    ...
    # trap check: over-cap goods + zero food can always forage
    def test_zero_food_over_cap_goods_may_gather_food(self):
        assert gather_rejection("wild_berries", {"stone": 65}, 0, 65) is None

    def test_unknown_kind_is_goods(self):
        assert (gather_rejection("mythril", {"mythril": 20}, 0, 20)
                == GATHER_REJECT_GOODS)
...
# ---------------------------------------------------------------------------
# Runtime wiring: gather cap and famished build gate
# ---------------------------------------------------------------------------


class TestGatherCapRuntime:
    ...
    def test_over_cap_goods_zero_food_may_still_forage(self, tmp_path):
        ...
        assert load_inventory(store)["east_adam"]["wild_berries"] == 1

    def test_over_cap_food_net_zero_replacement(self, tmp_path):
        store = _fresh_store(tmp_path)
        rt = _runtime(store)
        ...
        # but a second gather would grow the over-cap ledger -> rejected
