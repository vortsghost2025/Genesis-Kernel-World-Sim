"""Phase 10BF - shared public ignored event count equality contract proof.

These tests prove that a Phase 10AS two-agent public merge artifact can
be wrapped in a deterministic, sanitized shared-public-ignored-event-
count-equality contract that formalizes whether two agents' public
ignored event counts are identical, without ever inferring same ignored
...
Ignored event counts are supplied as optional caller arguments; 10BF
does not read them from 10AS bundles.  Missing, non-integer, or
negative ignored event counts are treated as ``None``.
...
10BF may say:
    ...
    "Both agents report the same public ignored_event_count value."
...
from backend.world.local_shared_public_ignored_event_count_equality_contract import (  # noqa: E402
    contract_ignored_event_count_equality_file,
    create_shared_ignored_event_count_equality_contract,
    export_shared_ignored_event_count_equality_contract,
)
...
# fabricate valid 10AS merge inputs.  The 10BF MODULE under test
# (local_shared_public_ignored_event_count_equality_contract.py) does not
...
# test_phase10bd_shared_public_movement_count_equality_contract.py, and
...
# ---------------------------------------------------------------------------
# Fixtures: build real 10AS merges to feed 10BF
# ---------------------------------------------------------------------------
...
class TestCreateSharedIgnoredEventCountEqualityContract:
    """Tests 1-21 from the 10BF spec test plan."""

    # 1. Happy path: both caller-supplied ignored event counts equal
    def test_happy_path_equal_counts(self):
        merge = _build_merge()
        contract = create_shared_ignored_event_count_equality_contract(
            merge,
            agent_a_ignored_event_count=1,
            agent_b_ignored_event_count=1,
        )
        assert contract["ok"] is True
        assert contract["contract_schema_version"] == "10BF.1"
        assert contract["contract_type"] == "shared_public_ignored_event_count_equality_contract"
        assert contract["contract_id"].startswith("10BF-")
        assert contract["claim_scope"] == "shared_public_ignored_event_count_equality_only"
        assert contract["errors"] == []
        assert contract["same_ignored_event_count"] is True
        assert contract["shared_ignored_event_count"] == 1

    # 2. Different caller-supplied ignored event counts
    def test_different_ignored_event_counts(self):
        merge = _build_merge()
        contract = create_shared_ignored_event_count_equality_contract(
            merge,
            agent_a_ignored_event_count=1,
        ...
        assert contract["ok"] is True
        assert contract["same_ignored_event_count"] is False
        assert contract["shared_ignored_event_count"] is None
        assert contract["agent_a_ignored_event_count"] == 1
        assert contract["agent_b_ignored_event_count"] == 5

    # 3. One caller-supplied ignored event count missing/None
    def test_one_bundle_missing_ignored_event_count(self):
        merge = _build_merge()
        contract = create_shared_ignored_event_count_equality_contract(
            merge,
            agent_a_ignored_event_count=None,
            agent_b_ignored_event_count=1,
        )
        assert contract["ok"] is True
        assert contract["agent_a_ignored_event_count"] is None
        assert contract["agent_b_ignored_event_count"] == 1
        assert contract["same_ignored_event_count"] is False
        assert contract["shared_ignored_event_count"] is None

    # 4. Both caller-supplied ignored event counts missing/None
    def test_both_bundles_missing_ignored_event_count(self):
        merge = _build_merge()
        contract = create_shared_ignored_event_count_equality_contract(
            merge,
            agent_a_ignored_event_count=None,
            agent_b_ignored_event_count=None,
        )
        assert contract["ok"] is True
        assert contract["agent_a_ignored_event_count"] is None
        assert contract["agent_b_ignored_event_count"] is None
        assert contract["same_ignored_event_count"] is False
        assert contract["shared_ignored_event_count"] is None
    ...
    def test_contract_id_deterministic(self):
        merge = _build_merge()
        c1 = create_shared_ignored_event_count_equality_contract(
            merge,
            agent_a_ignored_event_count=1,
            agent_b_ignored_event_count=1,
        )
        c2 = create_shared_ignored_event_count_equality_contract(
            merge,
            agent_a_ignored_event_count=1,
            agent_b_ignored_event_count=1,
        )
    ...
    def test_contract_id_changes_with_count(self):
        ...
        assert c1["same_ignored_event_count"] != c2["same_ignored_event_count"]
        assert c1["contract_id"] != c2["contract_id"]
    ...
    def test_structural_failures(self):
        # Non-dict
        c = create_shared_ignored_event_count_equality_contract("not a dict")
        ...
        bad_merge = {"ok": False, "merge_type": "two_agent_public_merge",
                     ...
                     "agent_b": {}}
        c = create_shared_ignored_event_count_equality_contract(bad_merge)
        assert c["ok"] is False
    ...
    # 10. Type validation: ignored_event_count not an integer
    def test_type_validation_non_integer_ignored_event_count(self):
        merge = _build_merge()
        contract = create_shared_ignored_event_count_equality_contract(
            merge,
            agent_a_ignored_event_count="one",
            agent_b_ignored_event_count=1,
        )
        assert contract["ok"] is True
        assert contract["agent_a_ignored_event_count"] is None
    ...
    def test_private_markers_redacted(self):
        ...
        exported = export_shared_ignored_event_count_equality_contract(contract)
        assert "/home/user/secret" not in exported

    # 12. Graceful handling of missing/None ignored event counts
    def test_graceful_missing_ignored_event_counts(self):
        merge = _build_merge()
        contract = create_shared_ignored_event_count_equality_contract(
            merge,
            agent_a_ignored_event_count=None,
    ...
    def test_export_stable_json(self):
        ...
        exported2 = export_shared_ignored_event_count_equality_contract(parsed)
        assert exported == exported2

    # 14. contract_ignored_event_count_equality_file reads and builds
    def test_contract_ignored_event_count_equality_file(self):
        merge = _build_merge()
        with tempfile.TemporaryDirectory() as tmpdir:
            path = Path(tmpdir) / "merge.json"
            path.write_text(json.dumps(merge), encoding="utf-8")
            result = contract_ignored_event_count_equality_file(
                path,
                agent_a_ignored_event_count=1,
                agent_b_ignored_event_count=1,
            )
        assert result["ok"] is True
        assert result["agent_a_ignored_event_count"] == 1
        assert result["agent_b_ignored_event_count"] == 1
    ...
    def test_all_public_functions_exercised(self):
        ...
        exported = export_shared_ignored_event_count_equality_contract(c1)
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            path = Path(tmpdir) / "merge.json"
            path.write_text(json.dumps(merge), encoding="utf-8")
            c2 = contract_ignored_event_count_equality_file(
                path,
                agent_a_ignored_event_count=1,
                agent_b_ignored_event_count=1,
            )
        assert c2["ok"] is True
        assert c2["same_ignored_event_count"] is True
    ...
    def test_module_has_no_forbidden_imports(self):
        module_path = (
            ...
            / "backend"
            / "world"
            / "local_shared_public_ignored_event_count_equality_contract.py"
        )
        ...
        for token in forbidden:
            assert token not in prose, (
                "forbidden token in 10BF module source: " + token
            )
    ...
    def test_module_does_not_call_upstream_creators(self):
        ...
        for creator in creators:
            assert creator not in prose, (
                "10BF module must not call upstream creator: " + creator
            )
...
class TestIgnoredEventCountBoundaries:
    """Additional boundary cases for 10BF."""

    # 19. Negative ignored_event_count treated as None
    def test_negative_ignored_event_count_treated_as_none(self):
        merge = _build_merge()
        contract = create_shared_ignored_event_count_equality_contract(
            merge,
            agent_a_ignored_event_count=-1,
    ...
    # 20. Zero ignored_event_count handled correctly
    def test_zero_ignored_event_count_handled(self):
        merge = _build_merge()
        contract = create_shared_ignored_event_count_equality_contract(
            merge,
            agent_a_ignored_event_count=0,
            agent_b_ignored_event_count=0,
        )
        assert contract["ok"] is True
        assert contract["agent_a_ignored_event_count"] == 0
        assert contract["agent_b_ignored_event_count"] == 0
        assert contract["same_ignored_event_count"] is True
        assert contract["shared_ignored_event_count"] == 0
...
class TestIgnoredEventCountScalarOnly:
    """Additional scalar-only boundary cases for 10BF."""

    # 10BF is scalar-only: no lists, no dedup, no set algebra.
    ...
    def test_no_list_fields_in_output(self):
        ...
        for key, value in contract.items():
            ...
            assert not isinstance(value, list), (
                "10BF is scalar-only; field " + key + " must not be a list"
            )
    ...
    def test_counts_not_present_beyond_count_fields(self):
        ...
        forbidden_fields = [
            "agent_a_ignored_event_count_count",
            "agent_b_ignored_event_count_count",
            "shared_ignored_event_count_count",
        ]
        for field in forbidden_fields:
            assert field not in contract, (
                "10BF is scalar-only; count field " + field + " must not exist"
            )
